...
# get tasks user has not responded yet
 ...
 def get_last_reviewed (user)
  ...
  def get_team_completed_tasks (user, remove_task_id)
     ...
     #r =  Activity.where("parent_id = ? AND NOT owner_id = ? AND  status_id = ? AND  activities.updated_at < ? AND NOT activities.id = ? ", self.parent_id,user.get_id, TivitStatus.get_completed_id, last_reviewed,remove_task_id).order(:completed_at).reverse_order
